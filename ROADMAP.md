...
| **Layer hatch** | A `hatch` field on `Layer`, used as the default by the paper-space `hatch` command (`commands/drafting.py:431`). Both things that stood in the way are gone: a detail's section now comes back from `g.section_regions()` + `g.face_loops()` as regions, and `Hatch` carries `holes`, so a bore stays a bore on screen, on the plot and under a Region-mode click. What is left is the layer field itself and the panel cell to set it. `core/layout.py hatch_region()` is what draws it. |
