...
| **ArrayPath orientation** (Freeform, etc.) | ✅ **Done.** Orientation option on `arraypath`: Freeform turns the copy with the curve in all three axes, Roadlike swings it round in plan but keeps it upright on the construction plane, None is the old pure translation. Frames from `geometry.sample_curve_frames`, a rotation-minimising frame (Wang's double reflection) rather than Frenet, whose normal is undefined on a straight stretch and flips at an inflection. |
