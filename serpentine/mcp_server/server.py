"""Serpentine MCP server.

Runs as a stdio MCP process and forwards tool calls to a running Serpentine
GUI via its localhost RPC bridge. Start Serpentine first (`serp`), then
register this server with your MCP client:

    claude mcp add serpentine -- serp-mcp
"""
...
mcp = FastMCP(
    "serpentine",
    instructions=(
        "Control the Serpentine NURBS 3D modeller. Objects are referenced "
...
class _Bridge:
    ...
    def connect(self):
        port = os.environ.get("SERP_RPC_PORT")
        if not port:
            try:
                port = open(PORT_FILE).read().strip()
            except OSError as exc:
                raise RuntimeError(
                    "Serpentine is not running (no RPC port file). "
                    "Launch the app with `serp` first.") from exc
    ...
    def call(self, method: str, **params) -> Any:
        for attempt in (1, 2):
            ...
            try:
                ...
                if attempt == 2:
                    raise RuntimeError(
                        "Lost connection to Serpentine — is the app running?")
...
def serp_command(command: str, inputs: list[str] | None = None) -> str:
    """Run any Serpentine command exactly as if typed in the command line,
