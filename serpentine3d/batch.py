"""Headless batch runner:  serp3d-batch script.py [args...]

The script runs with these names predefined:
    doc   — a serpentine3d.scripting.Document (fresh, or opened via --open)
...
def main(argv: list | None = None) -> int:
    ...
    if not os.path.exists(script_path):
        print(f"serp3d-batch: script not found: {script_path}",
              file=sys.stderr)
        return 2
